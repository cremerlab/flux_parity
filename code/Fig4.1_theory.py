...
for i, nu in enumerate(nu_max):
    ...
    init_params = [M0, out[1]/out[0] * M0,
                   out[2]/out[0] * M0, out[-2], out[-1]]
    init_integration = scipy.integrate.odeint(growth.model.self_replicator_FPM,
                                              init_params, time, args=(args,))
    _df = pd.DataFrame(init_integration, columns=[
                       'M', 'M_Rb', 'M_Mb', 'TAA', 'TAA_star'])
    _df['time_hr'] = time
    _df['balance'] = _df['TAA_star'] / _df['TAA']
    _df['MRb_M'] = _df['M_Rb'] / _df['M']
    _df['phi_Rb'] = (1 - const['phi_O']) * \
        _df['balance'] / (_df['balance'] + tau)
...
ax[2].set_ylim([0, 3])
ax[1].set_ylim([0, 3])
ax[1].hlines(1, 0, 2, linestyle='--',
             color=colors['primary_red'], lw=1.5, zorder=1000)
ax[2].hlines(1, 0, 2, linestyle='--',
             color=colors['primary_red'], lw=1.5, zorder=1000)
...
for g, d in relax_df.groupby(['nu']):
    ax[1].plot(d['time_hr'], d['phi_Rb'] -
               ss_phiRb, '-', color=cmap[i], lw=1.5)
