...
for i, nu in enumerate(nu_max):
    ...
    _df = pd.DataFrame(out, columns=['ribos', 'metabs', 'c_AA', 'c_N'])
    _df['biomass'] = _df['ribos'].values + _df['metabs'].values
...
for a in ax:
    ...
    a.set_xlabel(r'time [hr]', fontsize=8, color=colors['primary_black'])
...
for g, d in df.groupby('nu_max'):
    ax[0].plot(d['time'], d['rel_biomass'], '-', lw=0.75, color=palette[count])
    ax[1].plot(d['time'], d['c_AA'] / Kd_cAA, '-', lw=0.75, color=palette[count])
