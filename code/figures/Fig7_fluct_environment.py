...
total_time = 8
shift_time = 2
phi_O = np.linspace(0, 0.5, 20) 
palette = sns.color_palette('crest', n_colors=len(phi_O))
...
for i, phiO in enumerate(tqdm.tqdm(phi_O)):
        ...
        postshift_args = (gamma_max, nu_shift, tau, Kd_TAA_star, Kd_TAA, 0, 
                          kappa_max, 0, False, True, True)
...
for g, d in dynamic_df.groupby(['set_phiO']):
        gr = np.log(d['total_biomass'].values[1:]/ d['total_biomass'].values[:-1]) / 0.0001
        ax[0,0].plot(d['time'], d['phi_O'], color=palette[counter])
        ax[0,1].plot(d['time'], (d['total_biomass']), color=palette[counter])
        ax[1,0].plot(d['time'], d['prescribed_phiR'], color=palette[counter])
        ax[1,1].plot(d['time'], d['realized_phiR'], color=palette[counter])
        ax[2,0].plot(d['time'].values[1:], gr, color=palette[counter])

        counter += 1
ax[0,0].set(xlabel='time [hr]',
           ylabel='$\phi_O$')
ax[0, 1].set(xlabel='time [hr]',
          ylabel = '$M_O/M$')
ax[2,0].set(xlabel='time [hr]',
           ylabel='instantaneous growth rate $\lambda$ [hr$^{-1}$]')
ax[1,0].set(xlabel='time [hr]',
           ylabel='$\phi_{Rb}$')
ax[1,1].set(xlabel='time [hr]',
           ylabel='$M_{Rb}/M$')
