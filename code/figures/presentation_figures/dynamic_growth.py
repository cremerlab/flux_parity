...
dfs = []
nu_range = np.linspace(0.005, 10, 10)
...
for g, d in df.groupby('nu_max'):
    if ALL_NU == False:
        if g==nu_range[-1]:
            ax[0, 0].plot(d['time'], d['rel_biomass'], '-', lw=1, color=cmap[-1])
            ax[1, 0].plot(d['time'], d['gamma'], '-', lw=1, color=cmap[-1])
            ax[1, 1].plot(d['time'], d['nu'], '-', lw=1, color=cmap[-1])
            ax[2, 0].plot(d['time'], d['c_aa'], lw=1, color=cmap[-1])
            ax[0, 1].plot(d['time'], d['c_n'], lw=1, color=cmap[-1])
    else:
